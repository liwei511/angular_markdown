...
import {
  ALIGNED_LEFT_SYNTAX,
  ALIGNED_CENTER_SYNTAX,
  ALIGNED_RIGHT_SYNTAX,
} from './table-alignment-syntax';
      ...
      columnWidths
        .map(function(width, index) {
          let {
            ...
            postfix,
            adjust,
          } = calculateAlignmentMarkdownSyntaxMetadata(colAlignments[index]);
          return (
 ...
 */
export function calculateAlignmentMarkdownSyntaxMetadata(
  alignment: string,
): TableCellAlignment {
  switch (alignment) {
    case ALIGNED_LEFT:
 ...
 */
export function getColumnWidthsAndAlignments(
  rows: string[][],
): TableSizeMetadata {
...
export function looksLikeTable(data): boolean {
